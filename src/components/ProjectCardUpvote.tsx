...
function ProjectCardContainer({ children }: { children?: ReactNode }) {
  return (
    <div className="group px-4 py-2 text-tiny text-gray-light">
      {children}
    </div>
  );
}
  ...
  return (
    <ProjectCardContainer>
      {alreadyVoted && !votedThisSession && (
        <div className="flex items-center space-x-2 cursor-help" title="You already upvoted this project!">
      ...
      {!alreadyVoted && !votedThisSession && (
        <button
          ...
          disabled={!isMounted || upvoting}
          className="flex items-center space-x-2 group-hover:text-red cursor-pointer"
        >
          <Heart className="fill-transparent stroke-black group-hover:stroke-red" />

          <span>
            Votes: <span className="text-black group-hover:text-red">{project.votes}</span>
          </span>
        </button>
      )}
